...
def parse_restaurant_save(text):
    prompt = (
        ...
        f"- location: string (address or area)\n"
        f"- country: string (country, default 'Singapore' if not mentioned)\n"
        f"- tags: string (comma-separated tags — only if mentioned, else empty)\n"
        ...
        resp = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    "2. If yes, list up to 4 outlets as area names only.\n"
                    "3. If no, give the single area/neighbourhood.\n\n"
                    "Return ONLY a JSON object with:\n"
                    "- multiple_outlets: boolean\n"
                    "- outlets: list of area strings\n"
