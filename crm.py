...
import config
from sheets import crm_sheet, get_sheet


# --- Cache ---

def _invalidate_crm_cache():
    config._crm_cache = None

def _get_crm_records():
    if config._crm_cache is None:
        sheet = crm_sheet()
        config._crm_cache = sheet.get_all_records() if sheet else []
    return config._crm_cache
...
def find_row(name):
    """Single-pass CRM lookup. Returns (row_num, record) or ('disambig', message)."""
        ...
        for r in records:
            ...
            if fu:
                try:
                    ...
                    print(f"get_stats: bad follow up date: {e}")
            bday = r.get("Birthday", "")
            if bday:
                try:
                    ...
                    print(f"get_stats: bad birthday: {e}")
        return (
            f"📊 *CRM Stats*\n\n"
        ...
        for r in records:
            ...
            if fu_date:
                try:
                    ...
                    print(f"upcoming_followups: bad date: {e}")
        if not upcoming:
        ...
        for r in records:
            ...
            if fu_date:
                try:
                    ...
                    print(f"overdue_followups: bad date: {e}")
        if not overdue:
        ...
        for r in records:
            ...
            if bday_str:
                try:
                    ...
                    print(f"upcoming_birthdays: bad date: {e}")
        if not upcoming:
...
def check_birthday_acknowledgement(text):
    if not config.birthday_pending:
        return False, None
    lower = text.strip().lower()
    if lower not in ["sent", "done", "skip", "skipped", "sent it", "sent!"]:
        return False, None
    acknowledged = []
    for name, data in list(config.birthday_pending.items()):
...
# --- Scheduled Birthday Jobs ---

async def send_followup_reminders(app):
    try:
        records = _get_crm_records()
        today = date.today()
        from reminders import parse_date_flexible
    ...
    try:
        ...
        for i, r in enumerate(records):
            ...
            try:
                ...
                notes = r.get("Notes", "")
                relationship = r.get("Relationship", "")
                context_val = r.get("Context", "")

                already_greeted = r.get("Birthday Greeted", "")
                if already_greeted == today.strftime("%d/%m/%Y"):
                    continue

                greeting = _generate_birthday_greeting(name, age, relationship, context_val, notes)
...
async def send_birthday_followups(app):
    from config import YOUR_CHAT_ID
    try:
        still_pending = {k: v for k, v in config.birthday_pending.items() if not v.get("greeted")}
        ...
        config.birthday_pending = {}
    except Exception as e:
        print(f"Error in send_birthday_followups: {e}")


def _generate_birthday_greeting(name, age, relationship, context, notes):
    from clients import client
    greeting_prompt = (
        f"Write a warm, casual birthday greeting that someone could copy and paste to send to {name}, "
        f"who is turning {age} today.\n\n"
        f"Context: Relationship: {relationship}. Context: {context}. Notes: {notes or 'none'}.\n\n"
        f"Rules:\n"
...
async def handle_excel_import(file_bytes, column_order, update):
    ...
    try:
        ...
        existing_names_set = {r.get("Name", "").strip().lower() for r in existing_records if r.get("Name", "").strip()}

        col_map = {col_name.strip().lower(): i for i, col_name in enumerate(column_order)}
        ...
        for row in data_rows:
            ...
            birthday_raw_idx = col_map.get("date of birth") or col_map.get("dob") or col_map.get("birthday")
            birthday_raw = row[birthday_raw_idx] if birthday_raw_idx is not None and birthday_raw_idx < len(row) else None
            birthday = normalise_birthday(birthday_raw)

            alias = get_col(row, "alias")
            relationship = get_col(row, "relationship")
            context = get_col(row, "context")
            notes = get_col(row, "notes")
            email_raw = get_col(row, "email")
            email = str(email_raw).strip().lower() if email_raw else ""
            address = get_col(row, "address")
            referred_by = get_col(row, "referred by", "referral")
            referral_date = ""

            rows_to_append.append([
                name, alias, birthday, relationship, context, notes,
                "", "", today, "", referred_by, referral_date, email, address
            ])
